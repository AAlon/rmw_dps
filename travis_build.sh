...
docker exec osrf_ros2_nightly /bin/bash -c "apt-get update && apt-get install python-dev && source /opt/ros/dashing/setup.bash && rosdep update && rosdep install --from-paths /shared/rmw_dps --ignore-src -r -y"
